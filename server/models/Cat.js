...
/* While Mongo is a schema-less database, meaning we can just store arbitrary objects in it,
   ...
   The schema enforces a semi-rigid format on our data so that it is more of a
   "known quantity". This puts us somewhere between the very rigid SQL format and
   the very loose Mongo format.

   To create the schema we give it an object. Each top level key (like name,
   bedsOwned, and createdDate) defines a variable our objects can have. We then use
   individual objects to define the features and parameters of each one of those variables.
   ...
   added by Mongoose to clarify what each variable should contain. We also give it the
   required option, which means every object in the cat database MUST have a name. We set
   the trim option to true, which will remove trailing and leading whitespace from the
   string. Finally we set the unique option to true. This means that there cannot be two
   ...
   Rather than figure that out in our own code when we create each cat, we can instead set it's
   default to the Date.now function. When a new cat is made, it will execute the default function
   and return the current date.
*/
